...
export class MfaController {
  ...
  async verifyMFACode(@Body() verifyRecCodeDto: MfaVerifyDto, @Request() req: any) {
    if (req.user.accountStatus !== AccountStatus.ACTIVE) throw new UnauthorizedException()
    const isValid = await this.mfaService.verifyMFA(req.user._id, verifyRecCodeDto.mfaId, verifyRecCodeDto.code)
  ...
  async validateRecoveryCode(@Body() recoveryDto: MfaRecoveryDto, @Request() req: any) {
    if (req.user.accountStatus !== AccountStatus.ACTIVE) throw new UnauthorizedException()
    const isValid = await this.mfaService.validateRecoveryCode(req.user._id, recoveryDto.code)
  ...
  @Post('setup')
  @UseGuards(JwtAuthGuard)
  async setupMFA(@Body() setupDto: MfaSetupDto, @Request() req: any): Promise<MfaSetupResponseDto> {
    if (req.user.accountStatus !== AccountStatus.ACTIVE) throw new UnauthorizedException()
  ...
  @Post('setup/verify')
  @UseGuards(JwtAuthGuard)
  async verifyMFA(@Body() verifyDto: MfaVerifyDto, @Request() req: any): Promise<MfaSEnableResultDto> {
    if (req.user.accountStatus !== AccountStatus.ACTIVE) throw new UnauthorizedException()
    return this.mfaService.verifyAndEnableMFA(req.user._id.toString(), verifyDto.code, verifyDto.mfaId)
  }
