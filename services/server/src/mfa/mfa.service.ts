...
export class MfaService {
  ...
  // TODO: Move emailVerification to this service, or make three seperate modules or merge all these in a single module
  // FIXME: Deep nesting of services
  ...
  generateTOTPMFASecret(username: string): { secret: string; uri: string } {
    const base32Secret = speakeasy.generateSecret({
      name: 'ScholrFlow',
      issuer: 'ScholrFlow'
    })
  ...
  async verifyAndEnableMFA(
    ...
    if (userMfa.userId.toString() !== userId.toString())
      throw new BadRequestException('MFA_INVALID_USER')
    if (userMfa.type === MFAType.EMAIL) {
      if (!(await this.verifyEmailMFACode(_userId, code)))
        throw new BadRequestException('INVALID_CODE')
    }
    if (userMfa.type === MFAType.TOTP) {
      if (!this.verifyTOTPMFAcode(userMfa.secret, code))
        throw new BadRequestException('INVALID_CODE')
    }
  ...
  async getMFAStatus(userId: Types.ObjectId): Promise<IMfaStatusResponse | null> {
    ...
    return {
      mfaList: userMfa
        ...
        .map(mfa => ({ type: mfa.type, _id: mfa._id }))
    }
  }
